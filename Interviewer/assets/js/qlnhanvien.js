/* Chỉ điều khiển nội dung trang (#nvapp) – KHÔNG đụng header/sidebar */
const PALETTE = { blue:'#3E73FF', blue2:'#60a5fa', mint:'#17B890', orange:'#fb923c' };
const rgba = (hex,a)=>{ const n=parseInt(hex.replace('#',''),16),r=n>>16&255,g=n>>8&255,b=n&255; return `rgba(${r},${g},${b},${a})`; };

const scalesBase = {
  x:{ grid:{ color:'rgba(2,6,23,.06)' }, ticks:{ color:'#64748b', font:{ size:11, family:'Arial' } } },
  y:{ grid:{ color:'rgba(2,6,23,.06)' }, ticks:{ color:'#64748b', font:{ size:11, family:'Arial' } } }
};
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const EMP = [
    ...
    // thêm vài nhân sự để lọc phong phú hơn
    ['Đoàn Thanh Bình','Kỹ thuật','Backend Dev','binh.dt@corp.vn',1,'TB','b-blue',6],
    ['Tống Gia Hân','Phân tích DL','Data Analyst','han.tg@corp.vn',1,'GH','b-green',2],
    ['Hoài Vũ','Thiết kế','UX/UI','vu.hoai@corp.vn',1,'HV','b-green',5]
  ...
  const HIRED = [
    ...
    ['Ngô Kiều Anh','HR','15/02/2024','15–18 triệu','Trần Thị B','Hoàn thành'],
    ['Đỗ Tuấn Kiệt','QA','12/03/2024','18–22 triệu','Lê Hồng','Chờ onboard'],
    ['Lê Hà Anh','PM','22/03/2024','35–40 triệu','Nguyễn Văn A','Hoàn thành'],
    ['Phạm Khánh Ly','Fullstack Dev','02/04/2024','28–35 triệu','Trần Thị B','Đang thử việc'],
    ['Trần Tấn Phát','Backend Dev','12/04/2024','22–28 triệu','Lê Hồng','Hoàn thành']
  ].map(([n,r,s,sal,own,st],idx)=>({n,r,s,sal,own,st,i:n.split(' ').map(x=>x[0]).join('').slice(0,2),c: idx%2? 'b-green':'b-blue'}));

  // Tài khoản
  const ACC = [
    ['admin','admin@corp.vn','Admin',1,'20/08/2025 09:12'],
    ['hr01','hr01@corp.vn','HR',1,'19/08/2025 14:03'],
    ['hr02','hr02@corp.vn','HR',0,'—'],
    ['iv01','iv01@corp.vn','Interviewer',1,'18/08/2025 16:40'],
    ['iv02','iv02@corp.vn','Interviewer',1,'17/08/2025 08:20'],
    ['pm01','pm01@corp.vn','Admin',1,'21/08/2025 10:10'],
    ['lead01','lead@corp.vn','Admin',1,'20/08/2025 11:35'],
    ['tech01','tech01@corp.vn','Interviewer',1,'21/08/2025 12:05']
  ...
  if(g){
    charts.push(new Chart(g.getContext('2d'),{
      ...
      data:{
        ...
        datasets:[{
          label:'Số NV',
          data:[4,1,2,1,1,2,1,2,1],
          borderColor:PALETTE.blue, backgroundColor:rgba(PALETTE.blue,.12),
          fill:true, tension:.35, borderWidth:3, pointRadius:4, pointBackgroundColor:'#fff'
        }]
      },
  ...
  const makeDoughnut = (canvas, data, cutout='55%', pos=null)=>{
    ...
    const chart = new Chart(ctx,{
      ...
      options:{
        ...
        plugins:{legend:{position: pos ?? legendPos(), labels:{usePointStyle:true, boxWidth:10, font:{family:'Arial'}}}}
      }
    }); return chart;
  };
  ...
  const r = $('#roleChart');
  if(r){
    charts.push(makeDoughnut(r,{
      labels:['Trưởng phòng','HR','Backend Dev','Data Analyst','Kinh doanh','Tester','HR Assistant','Frontend','Fullstack Dev','Trưởng phòng NS'],
      datasets:[{ data:[10,8,12,10,8,5,6,7,6,4],
        backgroundColor:['#3b82f6','#f43f5e','#60a5fa','#22c55e','#f59e0b','#38bdf8','#a78bfa','#34d399','#93c5fd','#22d3ee'],
  ...
  const d = $('#deptChart');
  if(d){
    charts.push(makeDoughnut(d,{
      labels:['Phòng Dữ liệu','Phòng IT','Phòng Kế toán','Phòng Kinh doanh','Phòng Nhân sự'],
  ...
  const a = $('#ageChart');
  if(a){
    charts.push(new Chart(a.getContext('2d'),{
      type:'bar',
      data:{
        labels:['<25','25-30','31-35','36-40','41+'],
        datasets:[{ data:[1,3,9,2,0],
  ...
  /* ============ MODALS ============ */
  const showModal = (id)=>{ const m=$id(id); if(!m) return; m.classList.add('show'); m.setAttribute('aria-hidden','false'); };
  const hideModal = (id)=>{ const m=$id(id); if(!m) return; m.classList.remove('show'); m.setAttribute('aria-hidden','true'); };
  $$('.nv-modal [data-close]').forEach(btn=>btn.addEventListener('click', e=> hideModal(btn.closest('.nv-modal-backdrop').id)));
  $$('.nv-modal-backdrop').forEach(overlay=>{
    overlay.addEventListener('click', e=>{ if(e.target===overlay) hideModal(overlay.id); });
  });
  document.addEventListener('keydown', e=>{
    if(e.key==='Escape'){ $$('.nv-modal-backdrop.show').forEach(m=>hideModal(m.id)); }
  });
  ...
  function doAddEmp(){
    ...
    if(!n || !e){ alert('Vui lòng nhập họ tên và email.'); return; }
    const i=n.split(' ').map(x=>x[0]).join('').slice(0,2).toUpperCase();
    const c=['b-blue','b-green','b-purple'][Math.floor(Math.random()*3)];
    const rec={n,d,t,e,a,i,c,exp}; EMP.unshift(rec); state.current=[...EMP];
    renderEmp(state.current); hideModal('modalAddEmp');
  }
  $id('btnAdd')?.addEventListener('click', openAddEmp);
  $id('btnAddEmp')?.addEventListener('click', openAddEmp);
  $id('ae_save').addEventListener('click', doAddEmp);
  ...
  $id('pm_save').addEventListener('click', ()=>{
    ...
    if(!n || !e){ alert('Thiếu họ tên hoặc email.'); return; }
    const i=n.split(' ').map(x=>x[0]).join('').slice(0,2).toUpperCase();
    const c=['b-blue','b-green','b-purple'][Math.floor(Math.random()*3)];
  ...
  $id('ac_save').addEventListener('click', ()=>{
    ...
    if(!u || !email){ alert('Vui lòng nhập username & email'); return; }
    ACC.unshift({u,email,role,active,last:'—'}); renderAcc(ACC);
    hideModal('modalAccount');
  });

  /* Search nhanh cho 3 bảng */
  const runSearchEmp = ()=>{ const q=($id('empSearch').value||'').toLowerCase().trim();
    renderEmp(!q?state.current: state.current.filter(x=>[x.n,x.d,x.t,x.e].some(v=>String(v).toLowerCase().includes(q)))) };
  $id('empSearch').addEventListener('input', runSearchEmp);

  const runSearchHired = ()=>{ const q=($id('hiredSearch').value||'').toLowerCase().trim();
    renderHired(!q?HIRED: HIRED.filter(x=>[x.n,x.r,x.sal,x.own,x.st].some(v=>String(v).toLowerCase().includes(q)))) };
  $id('hiredSearch').addEventListener('input', runSearchHired);

  const runSearchAcc = ()=>{ const q=($id('accSearch').value||'').toLowerCase().trim();
    renderAcc(!q?ACC: ACC.filter(x=>[x.u,x.email,x.role].some(v=>String(x).toLowerCase().includes(q)))) };
  $id('accSearch').addEventListener('input', runSearchAcc);
});
